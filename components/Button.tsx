...
export const Button: React.FC<ButtonProps> = ({ 
  ...
  switch (variant) {
    ...
    case 'option':
      ...
      if (dynamicColor) {
        // Default: Very subtle, glass-like, dark text/border to blend in
        const baseState = "bg-white/5 border-white/10 text-gray-400 shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)]";
        
        // Interaction: Lights up intensely
        // Adjusted scaling: scale-105 on mobile, scale-110 on md+ to prevent overflow
        const interactionState = `
          ...
          hover:scale-105 md:hover:scale-110 hover:-translate-y-0.5 md:hover:-translate-y-1
          active:scale-95 active:duration-100
        `;

        // Active: "LED On" look - Strong glow + Inner light
        // Adjusted scaling: scale-105 on mobile, scale-110 on md+
        const activeState = isActive 
          ? "bg-[var(--dynamic-color)] border-[var(--dynamic-color)] text-make-dark shadow-[0_0_35px_var(--dynamic-color),inset_0_2px_10px_rgba(255,255,255,0.4)] scale-105 md:scale-110 -translate-y-0.5 md:-translate-y-1 z-20 font-bold"
          : baseState;
      ...
      // Standard Option Logic (For Text Choices)
        ...
        const activeClasses = isActive 
          ...
          : "bg-white/5 text-gray-300 border-white/10 shadow-[0_4px_6px_rgba(0,0,0,0.2)] hover:border-make-primary/50 hover:text-white hover:bg-white/10 hover:shadow-[0_0_15px_rgba(203,245,66,0.15)]";
        
        const clickFeedback = "active:scale-95 active:duration-75 active:brightness-110";
    ...
    case 'ghost':
      variantStyle = "bg-transparent text-gray-500 hover:text-white hover:bg-white/5 active:bg-white/10 active:scale-95 py-2 px-2 md:px-4 !rounded-xl text-xs md:text-sm font-medium transition-colors";
      break;
  }
